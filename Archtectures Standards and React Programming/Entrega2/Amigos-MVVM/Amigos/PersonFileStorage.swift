...
class PersonFileStorage : PersonDataStore {
    ...
    public func deletePerson(person toFind: Person) {
        ...
        for personJSON in results! {
            ...
            if let person = PersonFileStorage.parsePerson(json: personJSON) {
                if toFind == person {
        ...
        var jsonData: Data!
        do {
            jsonData = try JSONSerialization.data(withJSONObject: results!, options: JSONSerialization.WritingOptions())
